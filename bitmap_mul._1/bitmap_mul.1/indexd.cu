...
__global__ void kernel_index_bitmap(unsigned int **dbit,int dtotal,int dsize,int dheight,int dmul)
{
	int i,j;
	unsigned int num;
...
void cuda_malloc_cpy()
{
	...
	int thread_size=3;
	int block_size=1;
	if(attr_total%(thread_size*block_size)==0)
		mul=attr_total/(thread_size*block_size);
	else
		mul=(attr_total/(thread_size*block_size))+1;
	printf("mul:%d\n",mul);
	//int *dsize;
	//int *dlie;
	unsigned int **dbit;
	...
	kernel_index_bitmap<<<blocks,threads>>>(dbit,attr_total,attr_size,height,mul);
	printf("---------------T_T-------------\n");
